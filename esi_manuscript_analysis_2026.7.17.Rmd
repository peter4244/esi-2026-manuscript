...
bind_pid <- function(df, file_label) {
  ...
  if (!length(hit)) stop("No identifier column in ", file_label)
  df$pid <- as.character(df[[hit[1]]])
  df
}

esi_raw <- bind_pid(read.csv(ESI_PATH, stringsAsFactors = FALSE), "ESI_PATH")
phe_raw <- bind_pid(read.csv(PHE_PATH, stringsAsFactors = FALSE,
                             na.strings = c("", "NA")), "PHE_PATH")
vs      <- bind_pid(read.csv(VS_PATH,  stringsAsFactors = FALSE), "VS_PATH")
cod     <- bind_pid(read.csv(COD_PATH, stringsAsFactors = FALSE), "COD_PATH")
ex_raw  <- bind_pid(read.csv(EX_PATH,  stringsAsFactors = FALSE), "EX_PATH")
...
stopifnot(nrow(decl) == length(unique(decline_b$pid)))
