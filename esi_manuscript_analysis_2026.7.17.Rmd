...
disc_summary <- preserved %>%
  ...
  mutate(grp_5 = case_when(
    discord == "Bhatt-only-COPD" ~ "Bhatt-only-COPD (ESI missed)",
...
All three pairwise contrasts (Bhatt-only vs Both, Both vs ESI-only, Bhatt-only
...
disc_dat <- preserved %>%
  ...
  mutate(group = case_when(
    discord == "Bhatt-only-COPD" ~ "Bhatt-only",
...
pairs <- list(c("Bhatt-only","Both"), c("Both","ESI-only"),
              c("Bhatt-only","ESI-only"))
...
if (file.exists(CACHE_PATH)) {
  ...
  for (b in seq_len(B_BOOTSTRAP)) {
    ...
    fits <- tryCatch({
      list(
        all_b  = coxph(Surv(days_followed/365.25, vital_status) ~ bhatt_grp + age_visit + gender + race + SmokCigNow + ATS_PackYears + BMI, data = dat),
        all_e  = coxph(Surv(days_followed/365.25, vital_status) ~ esi_grp   + age_visit + gender + race + SmokCigNow + ATS_PackYears + BMI, data = dat),
        resp_b = coxph(Surv(days_followed/365.25, event_resp)   ~ bhatt_grp + age_visit + gender + race + SmokCigNow + ATS_PackYears + BMI, data = dat),
        resp_e = coxph(Surv(days_followed/365.25, event_resp)   ~ esi_grp   + age_visit + gender + race + SmokCigNow + ATS_PackYears + BMI, data = dat)
      )
    ...
    for (cat_ in cats) {
      store[["all_bhatt"]][b, cat_]  <- log_hr(fits$all_b,  paste0("bhatt_grp", cat_))
      store[["all_esi"]][b, cat_]    <- log_hr(fits$all_e,  paste0("esi_grp",   cat_))
      store[["resp_bhatt"]][b, cat_] <- log_hr(fits$resp_b, paste0("bhatt_grp", cat_))
      store[["resp_esi"]][b, cat_]   <- log_hr(fits$resp_e, paste0("esi_grp",   cat_))
    }
...
label_map <- c("Both-COPD"        = "Both-COPD",
               "Bhatt-only-COPD" = "Bhatt-only-COPD (ESI missed)",
...
Three comparisons among Both-COPD / Bhatt-only-COPD / ESI-only-COPD, using a
...
target_terms <- c("discordBoth-COPD", "discordBhatt-only-COPD", "discordESI-only-COPD")
...
make_K <- function(fit) {
  ...
  K <- rbind(
    "Both-COPD - Bhatt-only-COPD"       = row(1, 2),
    "Both-COPD - ESI-only-COPD"         = row(1, 3),
    "Bhatt-only-COPD - ESI-only-COPD"   = row(2, 3)
  )
  K
}
...
base_pres <- d_pres %>% transmute(
  rand_id,
  discord         = factor(discord, levels = c("Both-noCOPD", "Both-COPD", "Bhatt-only-COPD", "ESI-only-COPD")),
  Height_CM       = as.numeric(Height_CM),
  gender_baseline = factor(gender),
  race_baseline   = factor(race)
)
...
checks <- list(
  ...
  selfcheck("COPD-major resp HR (CT)",       t8_resp$bhatt_HR[t8_resp$group == "COPD-major"], 13.93, tol = 0.10),
  selfcheck("COPD-major resp HR (ESI)",      t8_resp$esi_HR[t8_resp$group == "COPD-major"],   11.85, tol = 0.10),
  ...
  selfcheck("Bhatt-only all-cause HR", t_grp_mort$all_HR[t_grp_mort$group  == "Bhatt-only-COPD (ESI missed)"], 1.53, tol = 0.02),
  selfcheck("ESI-only all-cause HR",   t_grp_mort$all_HR[t_grp_mort$group  == "ESI-only-COPD (Bhatt missed)"], 1.26, tol = 0.03),
  selfcheck("ESI-only CVD HR",         t_cs_disc$HR[t_cs_disc$cause == "CVD" & t_cs_disc$group == "ESI-only-COPD"], 2.74, tol = 0.05),
...
cause_cols <- c("CCOD_CVD","CCOD_Cancer","CCOD_Other")
