...
# Headless raster device. Two independent paths need one:
#   1. png() called directly by the figure chunks, which uses bitmapType. On a
#      node without X11 that defaults to Xlib, which cannot open a connection.
#   2. the device knitr uses for plots embedded in the report, set via `dev`.
# Fixing only (2) leaves the figures broken; fixing only (1) leaves knitr
# warning on every embedded plot. png() fails with a WARNING, not an error, and
# opts_chunk sets warning = FALSE, so either failure reads as a successful run.
...
# Probe the path the figure chunks actually use: base png().
local({
  .p <- file.path(tempdir(), "device_probe.png")
  # 200px with near-zero margins: a smaller canvas errors with "figure margins
  # too large", which looks like a device failure and is not.
  ok <- tryCatch({ png(.p, width = 200, height = 200)
                   op <- par(mar = rep(0.1, 4)); plot.new(); par(op); dev.off()
                   file.exists(.p) && file.size(.p) > 0 },
                 error = function(e) FALSE, warning = function(w) FALSE)
  if (!ok) stop("png() cannot write a file. bitmapType=", getOption("bitmapType"),
                " cairo=", capabilities("cairo"), " X11=", capabilities("X11"),
                ". Install cairo support or ragg.")
  unlink(.p)
  cat(sprintf("Raster device OK (png bitmapType=%s, knitr dev=%s)\n",
