...
# Headless raster device. Without X11 the default bitmap device is Xlib, which
# cannot open a connection: png() then fails with a WARNING, not an error, so
# with warning = FALSE above the run looks successful while writing no figure.
# Pick a device that needs no display, prefer ragg, and stop if none exists
...
if (requireNamespace("ragg", quietly = TRUE)) {
  ...
  stop("No headless raster device available: ragg is not installed, cairo is ",
       "not compiled in, and there is no X11 display. Install ragg.")
}
# Prove the chosen device actually opens, here, instead of finding out from a
...
local({
  ...
  # too large" and would look like a device failure.
  ok <- tryCatch({ png(.p, width = 200, height = 200)
                   op <- par(mar = rep(0.1, 4)); plot.new(); par(op); dev.off()
                   file.exists(.p) && file.size(.p) > 0 },
                 error = function(e) FALSE, warning = function(w) FALSE)
  if (!ok) stop("Raster device probe failed: png() cannot write a file. ",
                "bitmapType=", getOption("bitmapType"),
                " cairo=", capabilities("cairo"), " X11=", capabilities("X11"))
  unlink(.p)
  cat(sprintf("Raster device OK (dev=%s, bitmapType=%s)\n",
              knitr::opts_chunk$get("dev"), getOption("bitmapType")))
})
