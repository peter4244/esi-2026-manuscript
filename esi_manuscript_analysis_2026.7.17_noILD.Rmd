...
# Headless-safe raster device. On a Linux compute node without X11 the default
# bitmap device is Xlib, which cannot open a connection and makes every png()
# chunk fail with a warning rather than an error - so the run appears to
# succeed while silently producing no figure. cairo needs no display. This is
# deliberately conditional: macOS defaults to quartz, which already works
# headless, so overriding unconditionally would change local rendering for no
# reason.
if (identical(getOption("bitmapType"), "Xlib") && capabilities("cairo")) {
...
PAIRWISE_SEED  <- 20260717   # multcomp single-step Monte Carlo integration
...
stopifnot(nrow(cindex_tbl) == 2,
          setequal(cindex_tbl$outcome, c("all-cause", "respiratory")))
...
if (!is.null(.cached)) {
  ...
  boot_res <- list(
    ...
    # the ILD exclusion.
    fingerprint = list(n = nrow(mort_b),
                       deaths = sum(mort_b$vital_status == 1),
                       resp_events = sum(mort_b$event_resp == 1),
                       B = B_BOOTSTRAP),
    seed = BOOTSTRAP_SEED
