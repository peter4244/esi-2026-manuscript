...
We compared Cohen's κ, mean per-category sensitivity and macro-averaged F1 as
selection criteria across the threshold grids (Supplemental Table S4). For
NoCT-MD-COPD, κ selected a threshold of three symptom criteria, assigning 347
participants to COPD-minor, while balanced accuracy and macro-averaged F1 both
selected two, assigning 1,303; the reference contains 799. AFL-only contained
1,108 participants under every threshold examined, against 275 in the
reference. For ESI-MD-COPD the three criteria diverged: κ assigned 3
participants to AFL-only, balanced accuracy 954 and macro-averaged F1 543.
We used macro-averaged F1 for both classifications.
...
at ESI ≥ 1.50 for ESI-MD-COPD. ESI-MD-COPD achieved a held-out macro-averaged
F1 of 0.752 against 0.721 for NoCT-MD-COPD, a difference of 0.031 (95% across
folds: 0.014 to 0.044) {FIT-04a, FIT-04b, FIT-05a} (Supplemental Table S11).
Per-category F1 against MD-COPD was higher for ESI-MD-COPD in AFL-only, 0.47
against 0.40, and in COPD-major, 0.94 against 0.88, and marginally lower in
noCOPD and COPD-minor {F1CAT-01, F1CAT-02, F1CAT-03} (Table 2).

## Reclassification

*(new)*

All four classifications share the same major criterion, and no participant
moved between the airflow-limitation categories and the preserved-spirometry
categories under any of them. NoCT-MD-COPD reclassified 833 of 3,809 COPD-major
participants as AFL-only, 579 noCOPD participants as COPD-minor and 75
COPD-minor participants as noCOPD, and retained all 275 participants MD-COPD
assigned to AFL-only. ESI-MD-COPD reclassified 350, 612 and 70 respectively,
and retained 193, assigning the remaining 82 to COPD-major {RECL-01, RECL-02,
RECL-07, RECL-08}. The 833 participants NoCT-MD-COPD removed from COPD-major
were those whose only minor criterion was a CT finding, 833 of 3,809 (21.9%) of
that category {RECL-03}. In total, 7,753 participants (83.9%) retained their MD-COPD category
under NoCT-MD-COPD and 8,126 (87.9%) under ESI-MD-COPD (Figure 1, Supplemental
Table S10).
...
Crude rate ratios for the AFL-only category, against each classification's own
...
under MD-COPD {CRUDE-06, CRUDE-08, CRUDE-09}. Under NoCT-MD-COPD the
corresponding values were 1.54 (1.34 to 1.76), 9.38 (5.00 to 21.95) and 1.72
(1.43 to 1.98) {CRUDE-01, CRUDE-03}, and under ESI-MD-COPD 1.29 (1.05 to 1.55),
2.37 (0.44 to 6.99) and 1.17 (0.94 to 1.41) {CRUDE-04}. Adjusted estimates
followed the same ordering (Figures 2 to 4, Supplemental Tables S3a to S3c).

For the COPD-minor category the three multidimensional classifications gave
similar estimates. Crude all-cause rate ratios were 1.77, 1.72 and 1.75 for
MD-COPD, NoCT-MD-COPD and ESI-MD-COPD; respiratory 3.27, 3.42 and 3.89; and
...
Within the preserved-spirometry subgroup, 3,745 participants were classified as
not having COPD by both classifications, 612 by ESI-MD-COPD only, 70 by MD-COPD
only and 729 by both {DISC2-01, DISC2-02, DISC2-03}. ESI-MD-COPD agreed with
729 of 799 participants (91.2%) MD-COPD assigned to COPD-minor {DISC2-04}. Observed all-cause mortality was 1.46, 2.30, 2.13 and 2.84 per
100 person-years across these four groups, and exacerbations 11.2, 31.0, 21.7
and 42.9 (Table 3).
...
participants identified by ESI-MD-COPD alone had 1.59 (1.30 to 1.95), 3.63 (1.20
to 11.0) and 1.82 (1.48 to 2.25) {DISC2-05, DISC2-13, DISC2-07}. The 70
identified by MD-COPD alone had 1.25 (0.70 to 2.22) and 1.88 (1.05 to 3.35), and
recorded no respiratory deaths during follow-up, so no respiratory estimate was
obtained {DISC2-08}. Mean exacerbation count in the year before enrollment was
0.09, 0.44, 0.16 and 0.50 across the four groups {DISC2-09}.
...
Discrimination of the two criteria by ESI differed by stratum. Among
...
across the cohort the values were 0.78 and 0.80 {CTAUC-01}. FEV₁/FVC
discriminated both criteria at least as well as ESI in five of the six
stratum-by-criterion comparisons and equally in the sixth {CTAUC-02, CTAUC-02b}
(Supplemental Table S13).

Analyzed as a continuous measure, ESI was associated with all-cause mortality
in a model also containing FEV₁/FVC (hazard ratio 1.07 per unit, 95% CI: 1.03 to
...
than the noCOPD reference rather than faster.
